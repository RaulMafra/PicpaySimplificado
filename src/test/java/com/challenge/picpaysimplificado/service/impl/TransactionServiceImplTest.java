...
class TransactionServiceImplTest {
    ...
    @BeforeEach
    void init() {
        this.payer = new User(1L, "payer", "12345678909", "payer@email.com", "123", new BigDecimal(0), UserType.COMMON);
        this.receiver = new User(2L, "receiver", "12345678909632", "receiver@email.com", "123", new BigDecimal(0), UserType.COMPANY);
    ...
    void mustMakeTransactionWithSuccessfully() {
        ...
        Mockito.when(this.authorizationService.verifyAuthorization()).thenReturn(HttpStatus.OK);
        ...
        Mockito.verify(this.authorizationService).verifyAuthorization();
        ...
        Mockito.verify(this.sendNotification).send();
    ...
    @Test
    void mustFailedIfPayerIsCompany() {
        TransactionDTO transactionDTO = new TransactionDTO(new BigDecimal(10), 1L, 2L);
        ...
        assertAll(
                ...
                () -> assertEquals(new BigDecimal(0), this.receiver.getBalance()),
                () -> assertThrows(TransactionException.class, () -> this.transactionService.makePayment(transactionDTO),
                        "Company does not can make transfer")
        );
    ...
    @Test
    void musFailedifbalancepayerisinsufficient() {
        TransactionDTO transactionDTO = new TransactionDTO(new BigDecimal(10), 1L, 2L);
    ...
    void mustFailedIfAuthorizationServiceReturnStatusCodeDifferentThe200() {
        ...
        payer.setBalance(new BigDecimal(20));

        Mockito.when(this.userService.getUser(1L)).thenReturn(payer);
        Mockito.when(this.userService.getUser(2L)).thenReturn(receiver);


        Mockito.when(this.authorizationService.verifyAuthorization()).thenReturn(HttpStatus.BAD_REQUEST);

        assertAll(
                () -> assertEquals(new BigDecimal(20), this.payer.getBalance()),
                () -> assertEquals(new BigDecimal(0), this.receiver.getBalance()),
                () -> assertThrows(TransactionException.class, () -> this.transactionService.makePayment(transactionDTO),
                        "Transaction denied")
        );
        ...
        Mockito.verify(this.userService).getUser(2L);
        Mockito.verify(this.authorizationService).verifyAuthorization();
        Mockito.verifyNoInteractions(this.transactionRepository, this.sendNotification);
    }
